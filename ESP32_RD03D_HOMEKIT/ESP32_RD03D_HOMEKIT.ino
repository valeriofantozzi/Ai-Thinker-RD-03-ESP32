...
bool MULTI_TARGET = false;       // Can be toggled via serial command
...
// Radar Presence Sensor - detects if any target is present
struct RadarPresenceSensor : Service::OccupancySensor {
  SpanCharacteristic *occupancyDetected;
  
  RadarPresenceSensor() : Service::OccupancySensor() {
    occupancyDetected = new Characteristic::OccupancyDetected(0); // 0 = not detected, 1 = detected
    Serial.println("Configuring Radar Presence Sensor");
  }
  
  void updatePresence(bool hasTargets) {
    uint8_t currentValue = occupancyDetected->getVal();
    uint8_t newValue = hasTargets ? 1 : 0;
    
    if (currentValue != newValue) {
      occupancyDetected->setVal(newValue);
      Serial.print("Presence: ");
      Serial.println(hasTargets ? "DETECTED" : "NOT DETECTED");
    }
  }
};

// Radar Motion Sensor - detects if any target has speed > 0
struct RadarMotionSensor : Service::MotionSensor {
  SpanCharacteristic *motionDetected;
  
  RadarMotionSensor() : Service::MotionSensor() {
    motionDetected = new Characteristic::MotionDetected(false); // false = no motion, true = motion
    Serial.println("Configuring Radar Motion Sensor");
  }
  
  void updateMotion(bool hasMotion) {
    bool currentValue = motionDetected->getVal();
    
    if (currentValue != hasMotion) {
      motionDetected->setVal(hasMotion);
      Serial.print("Motion: ");
      Serial.println(hasMotion ? "DETECTED" : "NOT DETECTED");
    }
  }
};

// Global references to our sensors
RadarPresenceSensor* presenceSensor = nullptr;
RadarMotionSensor* motionSensor = nullptr;
...
void setup() {
  ...
  // Initialize HomeSpan Bridge
  homeSpan.begin(Category::Bridges, "Radar");
  ...
  new SpanAccessory();
    new Service::AccessoryInformation();
      ...
      new Characteristic::Manufacturer("Custom");
      new Characteristic::Model("RD03D Radar");
      new Characteristic::SerialNumber("RD03D-001");
  
  // Create Presence Sensor Accessory
  new SpanAccessory();
    new Service::AccessoryInformation();
      new Characteristic::Identify();
      new Characteristic::Name("Radar Presence");
    presenceSensor = new RadarPresenceSensor();
  
  // Create Motion Sensor Accessory  
  new SpanAccessory();
    new Service::AccessoryInformation();
      new Characteristic::Identify();
      new Characteristic::Name("Radar Motion");
    motionSensor = new RadarMotionSensor();
  ...
  Serial.println("HomeKit Bridge 'Radar' initialized");
  Serial.println("- Presence Sensor (OccupancySensor)");
  Serial.println("- Motion Sensor (MotionSensor)");
...
// Update HomeKit sensors based on radar data
void updateHomeKitSensors() {
  if (!presenceSensor || !motionSensor) return;
  
  // Check for presence (any active tracks)
  ...
  for (uint8_t i = 0; i < MAX_TRACKS; i++) {
    if (tracks[i].active) {
      hasTargets = true;
      // Check if this track has significant speed (motion)
      if (abs(tracks[i].speed) > 0.5) { // Speed threshold: 0.5 cm/s
        hasMotion = true;
      }
    }
  }
  
  // Update presence sensor
  presenceSensor->updatePresence(hasTargets);
  
  // Update motion sensor
  motionSensor->updateMotion(hasMotion);
}
...
void processSerialCommand() {
  if (Serial.available()) {
    ...
    if (command == "DEBUG") {
    ...
    else if (command == "HOMEKIT") {
      Serial.println("\n=== HOMEKIT STATUS ===");
      if (presenceSensor) {
        Serial.print("Presence: ");
        Serial.println(presenceSensor->occupancyDetected->getVal() ? "DETECTED" : "NOT DETECTED");
      }
      if (motionSensor) {
        Serial.print("Motion: ");
        Serial.println(motionSensor->motionDetected->getVal() ? "DETECTED" : "NOT DETECTED");
      }
      Serial.print("Active tracks: ");
      ...
      Serial.println("HOMEKIT - Show HomeKit sensor status");
...
void loop() {
  // HomeSpan polling for HomeKit communication
  homeSpan.poll();
  ...
  if(radar.update()) {
    ...
    updateZoneOccupancy();
    updateHomeKitSensors(); // Update sensors even without radar data
    delay(100);
  }
}
